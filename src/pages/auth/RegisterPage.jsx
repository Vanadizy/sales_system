...
const initialForm = {
  ...
  company: { name: '', businessType: 'Retail / General Merchandise', phone: '', email: '', address: '', country: 'Tanzania', currency: 'TZS', tin: '', website: '', receiptFooter: 'Thank you for your business.', logo: '' },
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        ...
        <form className="card" onSubmit={advance}>
          ...
          {step === 2 && <div className="grid gap-4 sm:grid-cols-2"><FormInput label="Company name" value={form.company.name} onChange={update('company', 'name')} required /><BusinessTypeSelect value={form.company.businessType} onChange={update('company', 'businessType')} showHint /><FormInput label="Company phone" value={form.company.phone} onChange={update('company', 'phone')} required /><FormInput label="Company email" type="email" value={form.company.email} onChange={update('company', 'email')} required /><FormInput label="Address" value={form.company.address} onChange={update('company', 'address')} required /><FormInput label="Country" value={form.company.country} onChange={update('company', 'country')} required /><Select label="Currency" value={form.company.currency} onChange={update('company', 'currency')}><option>TZS</option><option>USD</option><option>KES</option></Select><FormInput label="TIN / Registration number" value={form.company.tin} onChange={update('company', 'tin')} /><FormInput label="Website" value={form.company.website} onChange={update('company', 'website')} /><label className="block text-sm font-medium text-slate-700 dark:text-slate-200"><span className="mb-1.5 block">Company logo (optional)</span><input className="input" type="file" accept="image/*" onChange={upload('company', 'logo')} /></label><FormInput className="sm:col-span-2" label="Receipt footer message" value={form.company.receiptFooter} onChange={update('company', 'receiptFooter')} /></div>}
          {step === 3 && <div><div className="grid gap-4 sm:grid-cols-2"><FormInput label="Branch name" value={form.branch.name} onChange={update('branch', 'name')} required /><FormInput label="Branch code" value="Generated automatically" disabled /><FormInput label="Address / location" value={form.branch.address} onChange={update('branch', 'address')} required /><FormInput label="Phone" value={form.branch.phone} onChange={update('branch', 'phone')} required /></div><div className="mt-6 rounded-xl bg-brand-50 p-4 text-sm text-brand-950 dark:bg-brand-950 dark:text-brand-100"><p className="font-semibold">Ready to create your workspace</p><p className="mt-1">{form.owner.fullName} will be Company Admin and branch manager for {form.company.name} / {form.branch.name}.</p></div></div>}
